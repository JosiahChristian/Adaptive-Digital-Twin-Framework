...
AGGREGATE_OUTPUT_PATH = Path(
    "results/"
    "frozen_calibration_aware_controller_intervention.csv"
)

SEED_OUTPUT_PATH = Path(
    "results/"
    "frozen_calibration_aware_controller_intervention_seeds.csv"
)
...
CALIBRATION_RISK_THRESHOLD = 0.468010308717
...
def evaluate_seed(
    ...
    calibration_lookup = calibration_event_lookup(
        generation_seed
    )
    ...
    for index, row in enumerate(
        ...
        for action in newly_added:
            ...
            probability = calibration_probability(
                calibration_model,
                calibration_row,
            )
            ...
            if (
                ...
                if action in guarded_set:
                    guarded_set.remove(
                        action
                    )

                    vetoed_actions.append(
                        action
                    )
        ...
        guarded_action = responsive_action(
            guarded_set
        )
        ...
        if guarded_changed:
            if (
                ...
                guarded_outcome = "beneficial"
                guarded_beneficial += 1
            else:
                guarded_outcome = "harmful"
                guarded_harmful += 1
        ...
        baseline_calibration_probability = (
            calibration_probabilities.get(
                baseline_action,
                float(
                    "nan"
                ),
    ...
    guarded_metrics = evaluate_policy(
        test_rows,
        guarded_actions,
        true_best_actions,
        true_responsive_actions,
...
def main():
    ...
    print(
        f"historical calibration seeds="
    ...
    print(
        f"calibration risk threshold="
        f"{CALIBRATION_RISK_THRESHOLD:.12f}"
    )
    ...
    print(
        f"support safety threshold="
        f"{SAFETY_THRESHOLD:.2f}"
    )

    print(
        f"support downside threshold="
    ...
    print(
        "TRAINING FROZEN HISTORICAL CALIBRATION-RISK MODEL"
    )
    ...
    print(
        "BEGINNING UNTOUCHED PROSPECTIVE CONTROLLER BLOCK"
    )
    ...
    for row in aggregate_rows:
        ...
        print(
            ...
            f"{row['mean_veto']:.2f} "
            f"harmful_vetoed="
            f"{row['total_harmful_vetoed']} "
            f"beneficial_vetoed="
            f"{row['total_beneficial_vetoed']} "
    ...
    print(
        "calibration_guard - support_baseline"
    )

    print(
        f"mean dHarmful="
        f"{paired['mean_d_harmful']:+.3f}"
    )

    print(
        "harmful improved/unchanged/degraded="
        f"{paired['harmful_improved']}/"
        f"{paired['harmful_unchanged']}/"
        f"{paired['harmful_degraded']}"
    )

    print(
        f"mean dR="
        f"{paired['mean_d_regret']:+.6f}"
    )

    print(
        f"median dR="
        f"{paired['median_d_regret']:+.6f}"
    )

    print(
        f"range dR=["
        f"{paired['min_d_regret']:+.6f},"
        f"{paired['max_d_regret']:+.6f}"
        f"]"
    )

    print(
        "regret improved/unchanged/degraded="
        f"{paired['regret_improved']}/"
        f"{paired['regret_unchanged']}/"
        f"{paired['regret_degraded']}"
    )

    print(
        f"mean dUnder="
        f"{paired['mean_d_under']:+.3f}"
    )

    print(
        f"mean dBeneficial="
        f"{paired['mean_d_beneficial']:+.3f}"
    )

    print(
        f"mean dRetention="
        f"{paired['mean_d_retention']:+.3%}"
    )
    ...
    print(
        "A support-admitted action is vetoed only when:"
    )

    print(
        f"historical calibration-risk probability >= "
        f"{CALIBRATION_RISK_THRESHOLD:.12f}"
    )
    ...
    print(
        "The threshold was frozen using historical "
        "out-of-fold predictions from seeds 44001-44090 "
        "before prospective seeds 44091-44110 were evaluated."
    )

"""Experiment 165: exact prospective replication of intervention-aligned metric hierarchy."""
...
TEST=Path('results/prospective_action_conditioned_support_representation_actions_751_790.csv')
OUT=Path('results/preregistered_intervention_aligned_metric_hierarchy_replication.csv')
SEED_OUT=Path('results/preregistered_intervention_aligned_metric_hierarchy_replication_by_seed.csv')
...
def main():
    ...
    hierarchy=abs(r_recall)>abs(r_ap)>abs(r_auc)
    row={'seeds':len(s),'rho_recall_unsafe':r_recall,'rho_ap_unsafe':r_ap,'rho_auc_unsafe':r_auc,'absdiff_recall_minus_auc':obs_ra,'absdiff_recall_minus_ap':obs_rp,'boot_recall_minus_auc_p025':ci_ra[0],'boot_recall_minus_auc_p975':ci_ra[1],'boot_recall_minus_ap_p025':ci_rp[0],'boot_recall_minus_ap_p975':ci_rp[1],'criterion_observed_hierarchy':int(hierarchy),'criterion_boot_auc_above_zero':int(ci_ra[0]>0),'criterion_boot_ap_above_zero':int(ci_rp[0]>0)}
    row['primary_metric_hierarchy_replication_pass']=int(row['criterion_observed_hierarchy'] and row['criterion_boot_auc_above_zero'] and row['criterion_boot_ap_above_zero'])
    row['rho_recall_regret']=rho(s.delta_recall,s.delta_regret); row['rho_ap_regret']=rho(s.delta_ap,s.delta_regret); row['rho_auc_regret']=rho(s.delta_auc,s.delta_regret)
